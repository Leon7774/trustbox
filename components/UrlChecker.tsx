    ...
    <div className="w-full flex flex-col gap-6">
      <div className="relative flex items-center">
        ...
        <input
          ...
          className="w-full bg-trust-dark/50 border border-trust-border rounded-xl pl-12 pr-32 py-4 text-lg text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all font-mono"
        />
        <button
          onClick={() => url && analyzeUrl(url)}
          disabled={!url || isAnalyzing}
          className="absolute right-2 px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
          {isAnalyzing ? (
      ...
      {analysis && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="bg-trust-dark/30 rounded-xl p-6 border border-trust-border/50 flex flex-col md:flex-row gap-6 items-center justify-between relative overflow-hidden">
            ...
            <div className={`absolute top-0 right-0 w-64 h-64 blur-3xl rounded-full opacity-10 pointer-events-none ${
              analysis.score >= 80 ? "bg-trust-teal" : analysis.score >= 40 ? "bg-yellow-500" : "bg-red-500"
            }`} />
            
            <div className="flex items-center gap-6 z-10 w-full md:w-auto">
              <div className="relative w-24 h-24 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90">
                  <circle className="text-trust-dark" strokeWidth="8" stroke="currentColor" fill="transparent" r="40" cx="48" cy="48" />
                  <circle
                    className={`transition-all duration-1000 ${
                      analysis.score >= 80 ? "text-trust-teal" : analysis.score >= 40 ? "text-yellow-500" : "text-red-500"
                    }`}
                    strokeWidth="8"
            ...
            <div className="z-10 w-full md:w-auto flex-1 flex justify-end">
              {analysis.score >= 80 ? (
                <div className="flex items-center gap-2 text-trust-teal bg-trust-teal/10 px-4 py-2 rounded-lg border border-trust-teal/20">
          ...
          <div className="bg-trust-dark/30 rounded-xl border border-trust-border/50 overflow-hidden">
            <div className="p-4 border-b border-trust-border/50 bg-trust-surface hidden md:flex">
              <h3 className="text-white text-sm font-semibold">Structural Analysis</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-trust-border/50">
              <div className="p-5 space-y-4">
                <div className="flex items-start justify-between">
                  ...
                  {analysis.isHttps ? (
                    <span className="px-2 py-1 bg-trust-teal/20 text-trust-teal text-xs rounded border border-trust-teal/30">Secure</span>
                  ) : (
                ...
                <div className="flex items-start justify-between">
                  ...
                  {analysis.hasIpAddress ? (
                    ...
                    <span className="px-2 py-1 bg-trust-surface text-slate-400 text-xs rounded border border-trust-border">Valid Domain</span>
                  )}
                </div>
              </div>
              ...
              <div className="p-5 space-y-4">
                <div className="flex items-start justify-between">
                  ...
                  {analysis.hasSuspiciousKeywords ? (
                    ...
                    <span className="px-2 py-1 bg-trust-surface text-slate-400 text-xs rounded border border-trust-border">Clean</span>
                  )}
                </div>
                ...
                <div className="flex items-start justify-between">
                  ...
                  {analysis.isShortened ? (
                    ...
                    <span className="px-2 py-1 bg-trust-surface text-slate-400 text-xs rounded border border-trust-border">Direct</span>
                  )}
                </div>
              </div>
            </div>
